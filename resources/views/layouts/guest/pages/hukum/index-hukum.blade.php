...
@section('content')
    ...
    @include('components.breadcrumb', ['title' => 'Produk Hukum'])

    <section class="project-details-area pt-50 pb-120">
        <div class="container">

            <div class="section-title-two text-center mb-50">
                <span class="sub-title">Daftar Produk Hukum</span>
            </div>
                        ...
                        <div class="col-md-11">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($hukums as $key=>$k)
                                        <tr>
                                            <td class="text-center">{{ $key+1 }}</td>
                                            <td class="text-center">{{ $k->title }}</td>
                                            {{-- <td class="text-center">{{ date('d-m-Y', strtotime($k->date)) }}</td> --}}
                                            <td class="text-center"><a href="{{ route('detail.hukum', $k->slug) }}" class="btn btn-info btn-sm">Lihat</a></td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
